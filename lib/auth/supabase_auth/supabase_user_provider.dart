...
class CampusAppSupabaseUser extends BaseAuthUser {
  CampusAppSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> campusAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CampusAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
